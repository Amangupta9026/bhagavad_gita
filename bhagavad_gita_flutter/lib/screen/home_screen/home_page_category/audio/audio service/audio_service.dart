...
class AudioPlayerHandlerImpl extends BaseAudioHandler
  ...
  Future<void> _init() async {
    ...
    log('checking connectivity & setting quality');
    ...
    mediaItem.whereType<MediaItem>().listen((item) {
      ...
      if (item.artUri.toString().startsWith('http')) {
        //addRecentlyPlayed(item); // uncoment this to add to recently played
        ...
        if (recommend && item.extras!['autoplay'] as bool) {
          ...
          if (queueLength - index < 5) {
            log('less than 5 songs remaining, adding more songs');
            Future.delayed(const Duration(seconds: 1), () async {
              if (item == mediaItem.value) {
                if (item.genre != 'YouTube') {
                  ...
                  // log('Recieved recommendations: $res');
    ...
    try {
        ...
        if (lastQueueList.isNotEmpty &&
          ...
          if (lastQueue.isEmpty) {
            ...
            try {
              ...
              log('Error while setting last audiosource $e');
              await _player!.setAudioSource(_playlist, preload: false);
            }
          }
        } else {
      ...
      log('Error while loading last queue $e');
      await _player!.setAudioSource(_playlist, preload: false);
    }
    if (!jobRunning) {
      refreshJob();
  ...
  Future<void> refreshLink(Map newData) async {
    log('player | received new link for ${newData['title']}');
    ...
    //   }
    // }
    // log('player | inserting refreshed item');
  ...
  Future<void> startService() async {
    ...
    if (withPipeline && Platform.isAndroid) {
      ...
      log('starting without eq pipeline');
      _player = AudioPlayer();
    }
  }

  Future<void> addRecentlyPlayed(MediaItem mediaitem) async {
    log('adding ${mediaitem.id} to recently played');
    ...
    log('adding ${mediaitem.id} data to stats');
  ...
  Future<void> stop() async {
    ...
    log('caching last index and position');
    await Hive.box('cache').put('lastIndex', _player!.currentIndex);
    await Hive.box('cache').put('lastPos', _player!.position.inSeconds);
    await addLastQueue(queue.value);
  }
